...
"""
    ...
    def _execute_round(self, messages: List[Dict], system_content: str, tools: Optional[List], 
                      tool_manager, round_num: int) -> tuple[str, bool, List[Dict]]:
        """
        ...
        if response.stop_reason == "tool_use" and tool_manager:
            ...
            updated_messages = self._process_tool_calls(response, messages, tool_manager)
            ...
            if not should_continue:
                ...
                final_response = self._get_final_response(updated_messages, system_content, tools, tool_manager)
    ...
    def _process_tool_calls(self, response, messages: List[Dict], tool_manager) -> List[Dict]:
        """
    ...
    def _get_final_response(self, messages: List[Dict], system_content: str, tools: Optional[List], tool_manager=None) -> str:
        """
        ...
        if final_response.stop_reason == "tool_use" and tools and tool_manager:
            ...
            final_messages = self._process_tool_calls(final_response, messages, tool_manager)
    ...
    def _handle_error(self, error: Exception, round_num: int, messages: List[Dict], system_content: str) -> str:
        """
